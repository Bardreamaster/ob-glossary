...
class FolderSuggestModal extends FuzzySuggestModal<string> {
	...
	constructor(app: App, _initialPath: string, onChoose: (path: string) => void) {
		super(app);
		this.onChoose = onChoose;
		this.setPlaceholder(i18n_string("modal-folder-path-picker-placeholder"));
	}
...
export class ConceptNoteModal extends Modal {
	...
	onOpen() {
		...
		new Setting(contentEl)
			...
			.addText((text) =>
					text.setPlaceholder(i18n_string("modal-concept-name-placeholder")).onChange((value) => {
		...
		new Setting(contentEl)
			...
			.addText((text) =>
				text.setPlaceholder(i18n_string("modal-domain-placeholder")).onChange((value) => {
		...
		new Setting(contentEl)
			...
			.addText((text) =>
				...
				text)
					...
					.setPlaceholder(i18n_string("modal-folder-path-placeholder"))
					.onChange((value) => {
			...
			.addButton((button) =>
				button
					.setButtonText(i18n_string("modal-folder-path-select-button"))
					.onClick(() => {
						new FolderSuggestModal(this.app, this.result.folderPath, (path) => {
	...
	async createNewNote() {
		...
		try {
			...
			if (normalizedFolderPath) {
				...
				for (const segment of pathSegments) {
					...
					if (pathEntry && !(pathEntry instanceof TFolder)) {
						new Notice(i18n_string("modal-notice-invalid-folder-path"));
						return false;
					}
				...
				if (templateFile instanceof TFile) {
					...
					if (yamlMatch && yamlMatch[1]) {
						try {
							...
							console.error(i18n_string("modal-notice-template-yaml-error"), error);
							new Notice(i18n_string("modal-notice-template-yaml-error"));
						}
					...
					new Notice(
						i18n_string("modal-notice-template-file-not-found").replace("{path}", templatePath),
					);
				}
			}
			...
			const mergedDomain =
				inputProps.domain.length > 0 ? inputProps.domain : templateDomain;
			...
			new Notice(
				i18n_string("modal-notice-create-success").replace("{title}", normalizedTitle),
			);
			return true;
		} catch (error) {
